...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Скорость загрузки на SSD накопителе составила ~**20 секунд** до экрана входа в систему, что на самом деле не особо быстро, так как даже на чистый HDD жесткий диск Ubuntu запускается за меньшее время. После замены карты на **NVidia 770 GTX** скорость загрузки уменьшилась до 8 секунд.

SteamOS (под управлением gnome-shell) на запуске потребляет немного памяти, ~510MB ОЗУ(RAM) при входе в систему. С запущенным chromium 10-12 вкладками, с запущенным youtube стримом, со всеми моими дополнениями; с запущенным стим-клиентом памяти потреблялось 2GB. При сравнении с Ubuntu (под управлением unity) пустой рабочий стол в ней потребляет примерно 840Mb.

Нагрузка на процессор, сеть и жесткий диск без каких-либо видимых изменений в сторону утяжеления. Ubuntu в спокойном состоянии потребляет ~5% процессорного времени на каждом ядре/потоке. У SteamOS схожая ситуация, загрузка процессора в спокойном режиме составляет не более 5% на 1 ядро/поток.

В целом скорость работы SteamOS похожа на скорость любого другого свежеустановленного дистрибутива Linux или Windows, при поддержании порядка в системе и использовании стандартного окружения SteamOS будет работать также быстро через год, как и в момент последней настройки или установки софта в систему. Тем не менее, если собирать собственную сборку, то можно выиграть пару миллиметров оперативной памяти, что на данный момент, я считаю, не стоит потраченного времени.
...
A-AA-AAA тайтлы
```````````````
Все игры запускались изначально на самых высоких настройках, так как спецификации представленного железа сопостовимы с hi-end steam machine от Valve и 
...
Итоги
@@@@@
Исходя из всех тестовых запусков именно игр я пришел к целой серии выводов. Итак, сейчас Valve официально поддерживет железо только от NVidia, тестовые игры, которые я запускал на ``770 GTX 4GB RAM DDR5`` показали примерно одинаковый результат в разных играх как от Valve так и от сторонних компаний (к моему сожелению это было пока только Serious Sam 3). Игры же на ``R9 280x 3GB RAM DDR5`` работали несколько хуже в разных франшизах от Valve, хотя и не проседали настолько глубоко, чтобы пришлось снижать настройки. В целом же как nvidia так и amd честно отрабатывают как в SteamOS/Ubuntu так и в Windows 7. Тем не менее победу в производительности на **R9 280x** я бы отдал системе под управлением Windows 7. В случае Nvidia не все так очевидно и SteamOS вполне успешно конкурирует с главным ПК-шным игровым флагманом. На мой взгляд рановато бежать делать собственню Steam Machine, чтобы получить лучшую производительности из коробки, после установки SteamOS или любого другого Linux, в тоже время еще рано сбрасывать со счетов гонку за FPS'ами между Windows миром и Linux миром. В последнем случае и AMD и NVidia усердно трудятся над улучшением своих драйверов и это действительно видно по результатам.

К сожалению вся эта информация относится к видеокартам hi-end видекартам и железу. Если вы обладатель low или mid-end железа, то ситуация пока туманная, врываться в новый мир Linux для вас как мне кажется пока рано
